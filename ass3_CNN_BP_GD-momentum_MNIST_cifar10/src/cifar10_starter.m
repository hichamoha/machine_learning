function [y_test, pred] = cifar10_starter()
    addpath(genpath('./'));
    ...
    % elements of class 1 are consecutive training will not work on those 
    ...
    net.layers = {};
    net.layers{end+1} = struct('type', 'input', 'params',...
                               struct('size', [32, 32, 3]));
    
    % First conv layer
    net.layers{end+1} = struct('type', 'convolution', 'params',...
                                struct('weights',...
                                       0.1*randn(5,5,3,16)/sqrt(5*5*3/2),...
                                       'biases', zeros(16,1)),...
                                'padding', [2 2]);
    ...
    net.layers{end+1} = struct('type', 'convolution', 'params',...
                               struct('weights',...
                                      0.1*randn(5,5,16,16)/sqrt(5*5*16/2),...
                                      'biases', zeros(16,1)),...
                               'padding', [2 2]);
    net.layers{end+1} = struct('type', 'relu');
    net.layers{end+1} = struct('type', 'maxpooling'); % XXX
    ...
    % Fully connected MLP layers (Dense)
%     net.layers{end+1} = struct('type', 'fully_connected', 'params',...
%                                struct('weights',...
%                                       randn(10,4096)/sqrt(4096/2),...
%                                'biases', zeros(10,1)));
%     net.layers{end+1} = struct('type', 'softmaxloss');
    
    net.layers{end+1} = struct('type', 'fully_connected', 'params',...
                               struct('weights',...
                                      randn(10,1024)/sqrt(1024/2),...
                               'biases', zeros(10,1)));
    net.layers{end+1} = struct('type', 'softmaxloss');
