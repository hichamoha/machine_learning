function [res, param_grads] = evaluate(net, in, labels, varargin)
    ...
    for i=1:n_layers
        ...
        if i == 1
            assert(strcmp(layer.type, 'input'), ...
                'The first layer must be an input layer');
        end

        if strcmp(layer.type, 'input')
            assert(isequal(input_dims, layer.params.size),...
                           'The input dimension is wrong');
            res{i} = in;
        elseif strcmp(layer.type, 'fully_connected')
            assert(isfield(layer, 'params'),...
                'Parameters for the fully connected layer are not specified');
            assert(isfield(layer.params, 'weights'),...
                'The weights for the fully connected layer are not specified.');
            assert(isfield(layer.params, 'biases'),...
                'The biases for the fully connected layer are not specified.');
            
            res{i} = fully_connected_forward(res{i-1}, layer.params.weights, layer.params.biases);
        elseif strcmp(layer.type, 'convolution')
            assert(isfield(layer, 'params'), ...
                'Parameters for the convolutional layer are not specified');
            assert(isfield(layer.params, 'weights'),...
                'The weights for the convolutional layer are not specified.');
            assert(isfield(layer.params, 'biases'),...
                'The biases for the convolutional layer are not specified.');
            
            padding = [0 0];
            if isfield(layer, 'padding')
                padding = layer.padding;
            end
            res{i} = conv_with_padding_forward(res{i-1},...
                      layer.params.weights, layer.params.biases, padding);
    ...
    assert(numel(res{end}) == 1,...
        'The final output must be a single element, the loss.');
            ...
            elseif strcmp(layer.type, 'convolution')
                ...
                [grad{i}, param_grads{i}.weights, param_grads{i}.biases] ...
                    = conv_with_padding_backward(res{i-1}, ...
                                     grad{i+1}, layer.params.weights, ...
                                         layer.params.biases, padding);
            elseif strcmp(layer.type, 'maxpooling')
