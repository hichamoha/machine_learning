 function [y_test, pred] = mnist_starter()
    addpath(genpath('./'));
...
%     have size 14x14x16x(batch size)
    ...
    % Again conv layer2: 14x14x16x(batch size)
    net.layers{end+1} = struct('type', 'convolution', ...
                               'params',... 
                               struct('weights', ...
                                      randn(5,5,16,16)/sqrt(5*5*16/2),...
                                      'biases', zeros(16,1)), ...
                              'padding', [2 2]);
    ...
    net.layers{end+1} = struct('type', 'fully_connected',...
                               'params', ...
                               struct('weights', randn(10,784)/sqrt(784/2),...
                                      'biases', zeros(10,1)));
