% net = training(net, x, labels, x_val, labels_val, opts)
% Exercise 5: Implement gradient descent with momentum in  
...
function net = training(net, x, labels, x_val, labels_val, opts)
    ...
    for it = 1:opts.iterations
        ...
        if isnan(loss(it)) || isinf(loss(it))
            str1 = 'Loss is NaN or inf.';
            error( str1+'Decrease the learning rate or change the initialization.');
        end
            ...
            if isfield(layer, 'params')
                ...
                for k=1:numel(params)
                    ...
                    if isfield(opts, 'momentum')
                        ...
                        mu = opts.momentum;
                        momentum{i}.(s) = mu*momentum{i}.(s) + (1-mu)*grads{i}.(s); % XXXXXX
                        ...
                        % the momentum parameter is not specified
                        net.layers{i}.params.(s) = net.layers{i}.params.(s) - ...
