function L = softmaxloss_forward(x, labels)
    ...
    %        The loss for a single example is given in the assignment. Compute
    ...
    %{
    IND = sub2ind(siz,I,J) returns the linear index equivalent to the row 
    and column subscripts I and J for a matrix of size siz. siz is a 
    2-element vector, where siz(1) is the number of rows and siz(2) is 
    the number of columns.
    %}
    %error('Implement this!');
    xc = x(sub2ind(sz, labels', 1:batch));
    L = -xc + log(sum(exp(x)));
    L = mean(L);
    %L = mean(-x + log(sum(exp(x))));
%     Lsum = sum(-labels + log(sum(exp(x))));
%     L = mean(Lsum);
end
