...
hm = [-0.5,1];
hs = 0.5;
Xhippo = hs*randn(2,hn) + hm(:)*ones(1,hn);
Xhippo = [ones(1,hn); Xhippo];
Thippo = [0;0;1]*ones(1,hn);
...
plot(Xhippo(2,:),Xhippo(3,:),'*','MarkerSize',5)
...
plot(Xhippo(2,:),Xhippo(3,:),'k*','MarkerSize',5)
legend('y - dog','Dog','Cat','Hippo','Location','best')
hold off
...
plot(Xcat(2,:),Xcat(3,:),'ko','MarkerSize',5)
plot(Xhippo(2,:),Xhippo(3,:),'k*','MarkerSize',5)
legend('y - cat','Dog','Cat','Hippo','Location','best')
hold off

subplot(133), hold on,
contourf(x1grid,x2grid,y3mat,50)
plot(Xdog(2,:),Xdog(3,:),'k+','MarkerSize',5)
plot(Xcat(2,:),Xcat(3,:),'ko','MarkerSize',5)
plot(Xhippo(2,:),Xhippo(3,:),'k*','MarkerSize',5)
legend('y - hippo','Dog','Cat','Hippo','Location','best')
hold off


% Decision boundaries, per class
...
plot(Xcat(2,:),Xcat(3,:),'ko','MarkerSize',5)
plot(Xhippo(2,:),Xhippo(3,:),'k*','MarkerSize',5)
legend('max(y)','Dog','Cat','Hippo','Location','best')
hold off
