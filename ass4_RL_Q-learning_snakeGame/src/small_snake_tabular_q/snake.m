...
% 1: show everything, 
% 0: show nothing, 
% 0.1: show every tenth, and so on
show_fraction = 0; 
...
rewards = struct('default', -1, 'apple', 10, 'death', -1000);
...
for i = 1 : nbr_ep
    ...
    while 1
        ...
        % Update state.
        prev_score = score;
        [grid, score, reward, terminate] = ...
            update_snake_grid(grid, snake_len, score, rewards, action);
        ...
        if terminate
            ...
            sample = reward + 0;  % replace nan with something appropriate.
            ...
            if ~test_agent
                ...
                disp(['AVERAGE SCORE SO FAR:   ',...
                                       num2str(mean(all_scores(1 : i)))]);
                if i >= 10
                    disp(['AVERAGE SCORE LAST 10:  ',...
                                   num2str(mean(all_scores(i - 9 : i)))]);
                end
                if i >= 100
                    disp(['AVERAGE SCORE LAST 100: ',...
                                  num2str(mean(all_scores(i - 99 : i)))]);
                end
                if score > top_score
        ...
        next_state_idx = find(and(and(states(:, 1) == next_state(1),...
                                      states(:, 2) == next_state(2)), ...
                                  and(states(:, 3) == next_state(3),...
                                      states(:, 4) == next_state(4))));
