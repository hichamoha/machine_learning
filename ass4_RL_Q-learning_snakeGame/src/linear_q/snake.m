...
% Allowed to be changed. 1: show everything, 
%                        0: show nothing, 
...
for i = 1 : nbr_ep
    ...
    while 1
        ...
        if terminate
            ...
            target  = reward + 0; % replace nan with something appropriate
        ...
        [state_action_feats_future, prev_grid_show_future,...
            prev_head_loc_agent_future] = ...
            extract_state_action_features(prev_grid_show,...
                               grid_show, prev_head_loc_agent, nbr_feats);
        ...
        target = reward + gamm * ...
            max((Q_fun(weights, state_action_feats_future)));% replace nan with something appropriate
        pred = Q_fun(weights, state_action_feats, action) ; % replace nan with something appropriate 
        td_err  = target - pred; % don't change this
        weights = weights + alph * td_err * state_action_feats(:, action);  %(fill in blanks)
