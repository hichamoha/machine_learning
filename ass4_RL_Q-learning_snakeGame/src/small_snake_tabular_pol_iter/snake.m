...
try
    ...
    start_time                = tic;
    [states, next_state_idxs] = ...
        get_states_next_state_idxs(nbr_states, nbr_actions, N);
    end_time                  = toc(start_time);
    disp(['Done getting state and next state representations! Elapsed time: ',...
                                          num2str(end_time), ' seconds']);
...
% XXX CHECK OUT THE FUNCTION AND FILL IN THE BLANKS!
[values, policy, nbr_pol_iter, nbr_pol_eval] = ...
    policy_iteration(pol_eval_tol, next_state_idxs, rewards, gamm); 
end_time                                     = toc(start_time);
disp(['Policy iteration done! Number of policy iterations: ',...
                                           num2str(nbr_pol_iter)]);% ...
disp(['Number of policy evaluations: ', num2str(nbr_pol_eval),...
                    ', elapsed time: ', num2str(end_time), ' seconds']);
...
while 1
    ...
    state_idx = find(and(and(states(:, 1) == state(1),...
                             states(:, 2) == state(2)), ...
                         and(states(:, 3) == state(3),...
                             states(:, 4) == state(4))));
    ...
    % Update state.
    [grid, score, terminate] = update_snake_grid(grid, snake_len,...
                                                 score, action);
